...
fn update_data(conn: &Connection) -> Result<()> {
    ...
    if let Ok(mut existing_task) = existing_task {
        ...
        let _ = io::stdin().read_line(&mut priority).expect("Failed to read line");
        existing_task.priority = priority.trim().parse().unwrap_or(0);
        ...
        conn.execute(
            "UPDATE tasks SET title = ?1, description = ?2, due_date = ?3, priority = ?4, status = ?5 WHERE title LIKE ?6",
            &[&updated_task.title, &updated_task.description, &updated_task.due_date, &updated_task.priority.to_string(), &updated_task.status, &format!("%{}%", input_title.trim())],
        )?;
